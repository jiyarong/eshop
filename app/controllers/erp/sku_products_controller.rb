module Erp
  class SkuProductsController < BaseController
    ...
    def create
      @sku_product = @sku.sku_products.new(sku_product_attributes_from_raw_product)
    ...
    def sku_product_attributes_from_raw_product
      raw_product_key = params.require(:ec_sku_product).permit(:raw_product_key)[:raw_product_key]
    ...
    def ozon_product_options(store)
      ...
      RawOzon::Product
        ...
        .map do |product|
          [
            raw_product_option_label(store, product.ozon_product_id, product.offer_id, product.raw_json&.dig("sku"), product.name),
            "ozon:#{store.id}:#{product.ozon_product_id}"
          ]
        end
    end
    ...
    def wb_product_options(store)
      ...
      RawWb::Product
        ...
        .map do |product|
          [
            raw_product_option_label(store, product.nm_id, product.vendor_code, nil, product.title),
            "wb:#{store.id}:#{product.nm_id}"
          ]
        end
    end

    def raw_product_option_label(store, product_id, offer_id, platform_sku_id, product_name)
      [
        store.store_name,
        product_id,
        offer_id,
