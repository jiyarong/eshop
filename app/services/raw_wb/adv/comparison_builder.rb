module RawWb
  module Adv
    class ComparisonBuilder
      NEGATIVE_METRICS = %i[spend drr cpc cpo cpm avg_position].freeze
      ...
      def metric_comparison(current, previous, metric)
        ...
        { current:, previous:, delta_value: delta.round(2), delta_pct:, trend:, semantic: }
      end

      def none(current)
        { current:, previous: nil, delta_value: nil, delta_pct: nil, trend: "none", semantic: "none" }
      end

      def flat(current, previous)
        { current:, previous:, delta_value: 0, delta_pct: 0, trend: "flat", semantic: "neutral" }
      end
    end
  end
end
