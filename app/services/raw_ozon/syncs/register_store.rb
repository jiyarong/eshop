  ...
  module Syncs
    module RegisterStore
      def sync_register_store
        store = Ec::Store.find_or_create_by!(platform: 'ozon', ozon_raw_account_id: @account.id) do |s|
        ...
        credentials = {
          ...
          ozon_performance_client_id: @account.performance_client_id,
          ozon_performance_client_secret: @account.performance_client_secret,
        }
        stale = credentials.any? { |field, value| store.public_send(field) != value }
        store.update!(credentials) if stale
        1
      end
    end
  end
