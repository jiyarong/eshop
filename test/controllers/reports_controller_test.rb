...
class ReportsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "sku detail renders in a drawer and keeps drawer mode for internal navigation" do
    ...
    assert_select ".sku-detail-tabs" do
      assert_select "a", text: "生命周期", count: 0
  ...
  test "sku detail defaults to sales funnel while hidden lifecycle remains directly accessible" do
    event_count = Ec::SkuLifecycleEvent.count

    get report_sku_path(@sku.sku_code), headers: { "Accept" => "text/html" }

    assert_response :success
    assert_select ".sku-detail-tabs a[aria-current='page']", I18n.t("reports.sku_detail.tabs.sales_funnel")
    assert_select ".sku-detail-tabs a", text: I18n.t("reports.sku_detail.tabs.lifecycle"), count: 0

    sign_in @current_user
    get report_sku_path(@sku.sku_code),
      params: { tab: "lifecycle" },
      headers: { "Accept" => "text/html" }
  ...
  test "sku detail hides the legacy AI diagnosis launcher" do
    get "/reports/skus/#{@sku.sku_code}", headers: { "Accept" => "text/html" }

    assert_response :success
    assert_select "a.sku-detail-ai-diagnosis", count: 0
  ...
  test "sku detail renders platform product bindings through shared table" do
    ...
    assert_select "a[href=?][data-turbo-frame=?]", "/erp/platform_products/ozon/#{@sales_store.id}/#{binding.product_id}", "_top", "查看属性"
