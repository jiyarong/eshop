...
class Ec::OperatorSkuSalesFunnelMetricsQueryTest < ActiveSupport::TestCase
  ...
  test "loads multiple skus in a fixed number of queries and compares natural weeks" do
    @products.each_with_index do |product, index|
      create_funnel(product, Date.new(2026, 8, 25), views: 100, carts: 20, orders: 10)
      create_funnel(product, Date.new(2026, 9, 1), views: 120 + index, carts: 30, orders: 15)
  ...
  def create_funnel(product, date, views:, carts:, orders:)
    RawWb::SalesFunnelDaily.create!(
      account: @account, stat_date: date, nm_id: product.product_id,
      open_card: views, add_to_cart: carts, orders:, synced_at: Time.current
    )
  end
