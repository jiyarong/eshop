...
class GoogleSheets::WeeklySummaryDeepServiceTest < ActiveSupport::TestCase
  ...
  test "writes one aggregated row per sku with previous comparisons and derived metrics" do
    ...
    assert_equal 20.0, sku_a_row[15]
    assert_equal 30.0, sku_a_row[16]
    assert_equal 27, sku_a_row.size
  ...
  test "writes fee breakdown columns matching sku detail metrics" do
    current_rows = [
      ...
      { sku: "WSUDEEP-A", platform: "Ozon", shop: "OZ-1", net_sales: 3, revenue: 60, ads: 6, goods_cost: 18, pre_tax: 24, tax: 4, after_tax: 20,
        commission_fee: 2, payment_fee: 0.5, delivery_fee: 1, return_delivery_fee: 0.25, storage_fee: 0.75,
        dispatch_fee: 0.1, packing_fee: 0.15, defect_fee: 0.05, crossdock_fee: 0.2, other_platform_fee: 0.3 }
    ]

    writes = capture_sheet_writes(current_rows:, previous_rows: [])
    values = writes.fetch(:writes).find { |entry| entry[:range] == "WSU-DEEP:W22!A1" }.fetch(:values)

    assert_equal "销售佣金(CNY)", values[0][17]
    assert_equal "支付手续费(CNY)", values[0][18]
    assert_equal "物流费(CNY)", values[0][19]
    assert_equal "退货物流费(CNY)", values[0][20]
    assert_equal "仓储费(CNY)", values[0][21]
    assert_equal "退件费(CNY)", values[0][22]
    assert_equal "包装费(CNY)", values[0][23]
    assert_equal "瑕疵处理费(CNY)", values[0][24]
    assert_equal "越库费(CNY)", values[0][25]
    assert_equal "其它平台费(CNY)", values[0][26]

    row = values.find { |r| r[0] == "WSUDEEP-A" }
    assert_equal 5.0, row[17]
    assert_equal 1.5, row[18]
    assert_equal 3.0, row[19]
    assert_equal 0.75, row[20]
    assert_equal 2.25, row[21]
    assert_in_delta 0.3, row[22], 0.001
    assert_in_delta 0.45, row[23], 0.001
    assert_in_delta 0.15, row[24], 0.001
    assert_in_delta 0.6, row[25], 0.001
    assert_in_delta 0.9, row[26], 0.001
    assert_equal 27, row.size
  end
