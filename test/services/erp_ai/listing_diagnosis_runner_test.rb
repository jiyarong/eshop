...
class ErpAI::ListingDiagnosisRunnerTest < ActiveSupport::TestCase
  ...
  test "runs listing-audit with weekly funnel and search term data for two complete weeks" do
    ...
    main_image_blob = Object.new
    merged_image_blob = Object.new
    ...
    listing_context.define_singleton_method(:image_attachments) do |sku_product:|
      ...
      [
        Struct.new(:file).new(file.new(main_image_blob)),
        Struct.new(:file).new(file.new(merged_image_blob))
      ]
    end
    ...
    assert_equal [ main_image_blob, merged_image_blob ], ask_arguments.fetch(:images)
  ...
  test "marks the diagnosis failed when the agent call raises" do
    ...
    listing_context.define_singleton_method(:image_attachments) { |**| [] }
    funnel_context = Class.new do
