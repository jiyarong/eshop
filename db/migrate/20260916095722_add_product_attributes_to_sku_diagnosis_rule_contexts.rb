class AddProductAttributesToSkuDiagnosisRuleContexts < ActiveRecord::Migration[8.1]
  OLD_CONTEXT_KEYS = %w[
  ...
  NEW_CONTEXT_KEYS = (OLD_CONTEXT_KEYS + %w[product_attributes]).freeze
  ...
  def up
    ...
    execute <<~SQL.squish
      ...
      SET configuration = jsonb_set(
        configuration,
        '{context_keys}',
        COALESCE(configuration -> 'context_keys', '[]'::jsonb) || '["product_attributes"]'::jsonb
      )
      WHERE NOT (COALESCE(configuration -> 'context_keys', '[]'::jsonb) ? 'product_attributes')
    SQL
  end
  ...
  def down
    execute <<~SQL.squish
      ...
      SET configuration = jsonb_set(
        configuration,
        '{context_keys}',
        COALESCE(configuration -> 'context_keys', '[]'::jsonb) - 'product_attributes'
      )
    SQL
